...
class DockerfilePath:
    SOURCE = "/datadrive/app_monitoring/"
    TARGET = "/datadrive/app_monitoring/"
...
def restart_service(service_name, old_container_id, user_name_ip_map,ip_to_client_map):
    ...
    source = " -f " + DockerfilePath.SOURCE + "Dockerfile"
    target = DockerfilePath.TARGET
    ...
    try:
        cmd_remove = prefix + "docker container rm " + old_container_id
        os.system(cmd_remove)
    except Exception as e:
        print('cannot delete ', old_container_id, ' does not exist in machine:',ip, ' continuing...')

    # Run docker
    new_container = docker_client.containers.run(service_name,detach=True,name=service_name,environment={"KAFKA_ADDRESS":KAFKA_BROKER})
...
def check_service_status(collection, client, service_info, user_name_ip_map,ip_to_client_map): #(collection, client,service_info, user_name_ip_map,)
...
def check_status(ip, collection, service_info, user_name_ip_map,):
    ...
    check_service_status(collection, client,service_info, user_name_ip_map,ip_to_client_map)
